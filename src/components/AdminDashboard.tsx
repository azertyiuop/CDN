...
const AdminPanelEnhanced: React.FC<AdminPanelEnhancedProps> = ({
    ...
    const handleMessage = (event: MessageEvent) => {
      try {
        ...
        if (data.type === 'admin_data_update') {
          ...
          const normalized = data.streams.map((s: any) => ({
            ...s,
            url: buildProxyUrl(s.url)
          }));
          setM3uStreams(normalized);
          localStorage.setItem('m3u_streams', JSON.stringify(normalized));
  ...
  const handleAddStream = () => {
    ...
    const newStream = {
      id: `stream_${Date.now()}`,
      name: newStreamName,
      url: buildProxyUrl(newStreamUrl),
      type: 'm3u8',
      isActive: true,
    ...
    const updatedStreams = [...m3uStreams, newStream].map(s => ({ ...s, url: buildProxyUrl(s.url) }));
  ...
  const handleDeleteStream = (streamId: string) => {
    ...
    const updatedStreams = m3uStreams.filter(s => s.id !== streamId).map(s => ({ ...s, url: buildProxyUrl(s.url) }));
    setM3uStreams(updatedStreams);
    localStorage.setItem('m3u_streams', JSON.stringify(updatedStreams));
    // broadcast en temps réel
    window.dispatchEvent(new CustomEvent('m3u_streams_updated'));
